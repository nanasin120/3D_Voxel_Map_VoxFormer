...
class occupiedNet(nn.Module):
    ...
    def forward(self, x):
        # x : [B, 16, 64, 64]
        x = self.layer1(x) # x : [B, 16, 32, 32]
        x = self.layer2(x) # x : [B, 16, 32, 32]
        x_out = self.layer3(x) # x_out : [B, 16, 32, 32]
        return x_out # x_out : [B, 16, 32, 32]
...
class deformable_self_attention(nn.Module):
    ...
    def forward(self, query, ref_points, volume_features, mask):
        # query : [B, 5000, 128]
        # ref_points : [B, 5000, 3]
        # volume_features : [B, 128, 16, 128, 128]
        # mask : [B, 5000]
        ...
        # volume_features : [B, 128, 16, 128, 128] p_delta_p : [B, 5000, 8, 3]
        # ImageF : [B, 5000, 8, 128]
        ImageF = F.grid_sample(volume_features, p_delta_p.unsqueeze(3), mode='bilinear', align_corners=True)
...
class ResidualBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.conv2(x)
        x = self.bn2(x)
        x_out = self.relu(identity + x)
        return x_out

class VoxFormer(nn.Module):
    def __init__(self, d_model = 128, pred_num=4, image_height=192, image_width=624):
        ...
        self.backbone = nn.Sequential( # resnet18. 정보 응축
            ...
            # [128, 24, 77] 채널은 128, 크기는 1/16
        )
        self.neck = nn.Sequential( # resnet에서 나온 채널 128을 d_model로 만들어줌
        ...
        self.completion_head = nn.Sequential(
            nn.Conv3d(pred_num, pred_num * 8, kernel_size=3, padding=1),
            nn.BatchNorm3d(pred_num * 8),
            nn.ReLU(),

            ResidualBlock(pred_num * 8),
            ResidualBlock(pred_num * 8),

            nn.Conv3d(pred_num * 8, pred_num, kernel_size=1)
        )
        self.upsample = nn.ConvTranspose3d(pred_num, pred_num, kernel_size=(3, 4, 4), stride=(1, 2, 2), padding=(1, 1, 1))

        # 시간을 11초에서 4초로 일단 단축

        self.image_height = image_height # 원본 이미지 높이
        self.image_width = image_width # 원본 이미지 너비
        self.grid_height = image_height//8 # depthNet이후 이미지 높이
        self.grid_width = image_width//8 # depthNet이후 이미지 너비
    ...
    def forward(self, image, intrinics, rots, trans):
        ...
        image_features = self.backbone(image) # [B * N, 128, 24, 77]
        image_features = self.neck(image_features) # [B * N, d_model, 24, 77]
        
        depth_disparity = self.depthNet(image_features) # [B * N, 45, 24, 77]
        _, D, H_new, W_new = depth_disparity.shape 
        depth_disparity = depth_disparity.view(B, N, D, H_new, W_new) # [B, N, 45, 24, 77]

        geometry = self.get_geometry(intrinics, rots, trans) # geometry = [B, N, 45, 24, 77, 3] 좌우, 높이, 앞뒤
        voxelized_pseudo_point_cloud = self.voxelization(depth_disparity, geometry) # [B, 16, 64, 64]
        M_out = self.occupiedNet(voxelized_pseudo_point_cloud) # [B, 16, 32, 32]
        query_proposals = self.get_query_proposal(M_out)  # [B, 2500, 3] 높이, 좌우, 앞뒤
        # t1.record()
        ...
        # reference_points : [B, N, 2500, 2] [높이, 좌우] [현재 0 ~ 1인 상태]
        # mask : [B, N, 2500]
        reference_points, mask = self.get_reference_point(query_proposals, intrinics, rots, trans)

        # position_embedding : [B, 2500, d_model]
        position_embedding = self.position_encoder(query_proposals.float())
        
        # content_embedding : [B, 2500, d_model]
        content_embedding = self.query_embedding.weight.unsqueeze(0).expand(B, -1, -1)

        # query : [B, 2500, d_model]
        query = content_embedding + position_embedding

        batch_idx = torch.arange(B, device=query.device).view(B, 1).expand(B, self.k)
        grid_size = torch.tensor([16, 32, 32], device=query_proposals.device) # 높이, 좌우, 앞뒤
        reference_points_3d = query_proposals / (grid_size - 1.0) # 높이, 좌우, 앞뒤
        reference_points_3d = reference_points_3d[..., [1, 0, 2]]

        x_idx = torch.clamp(query_proposals[..., 1].long(), 0, 32 - 1) # 좌우
        y_idx = torch.clamp(query_proposals[..., 0].long(), 0, 16 - 1) # 높이
        z_idx = torch.clamp(query_proposals[..., 2].long(), 0, 32 - 1) # 앞뒤
        ...
        for i in range(3):
            # query : [B, 2500, d_model]
            # reference_points : [B, N, 2500, 2]
            # image_features [B * N, 128, 24, 77]
            # mask : [B, N, 2500]
        ...
        # t0.record()
        for i in range(2):
            voxel_volume = torch.zeros(B, 128, 32, 16, 32, device=query.device)
            voxel_volume[batch_idx, :, z_idx, y_idx, x_idx] = query
        ...
        low_res_grid = torch.zeros(B, 4, 16, 32, 32, device=logits.device)
        low_res_grid[batch_idx, :, y_idx, x_idx, z_idx] = logits

        out = self.upsample(low_res_grid) 
        out = out + self.completion_head(out) 
...
# --- Stage 1 ---
    ...
    def get_geometry(self, intrinics, rots, trans):
        ...
        frustum = self.frustum # [45, 24, 77, 3] 좌우, 높이, 앞뒤

        inv_intrincs = torch.inverse(intrinics)

        # point_c : [B, N, 45, 24, 77, 3, 1]
        ...
        # point_w = [B, N, 45, 24, 77, 3, 1]
        points_w = torch.matmul(rots.view(B, N, 1, 1, 1, 3, 3), points_c) + trans.view(B, N, 1, 1, 1, 3, 1)

        # point_w = [B, N, 45, 24, 77, 3] 좌우, 높이, 앞뒤
        return points_w.squeeze(-1)
    
    def voxelization(self, depth_disparity, geometry):
        # depth_disparity : [B, N, 45, 24, 77] 
        # geometry        : [B, N, 45, 24, 77, 3] [좌우, 높이, 앞뒤]
        B, N, D, H, W, _ = geometry.shape
        device = geometry.device

        geom = geometry.reshape(-1, 3) # [B * N * 45 * 24 * 77, 3]
        flat_depth_disparity = depth_disparity.reshape(-1) # [B * N * 45 * 24 * 77]

        nx = ((geom[:, 0] + self.voxel_x * self.voxel_size / 2) / self.voxel_size).long() # 좌우
        ny = ((geom[:, 1] + self.voxel_y * self.voxel_size / 2) / self.voxel_size).long() # 높이
        nz = ((geom[:, 2] + self.voxel_z * self.voxel_size / 2) / self.voxel_size).long() # 앞뒤
        ...
        voxels = voxels.view(B, self.voxel_x, self.voxel_y, self.voxel_z).permute(0, 2, 1, 3) # [B, 16, 64, 64] [높이, 좌우, 앞뒤]
...
# --- Stage 2 ---  
    ...
    def get_reference_point(self, query_proposals, intrinics, rots, trans):
        # query_proposal : [B, 2500, 3] [높이 좌우 앞뒤]
